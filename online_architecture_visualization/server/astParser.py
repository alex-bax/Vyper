...
class AstParser():
    ...
    def __init__(self):
        ...
        self.funcNameLister = FuncLister()
    ...
    def parseTree(self, tree):
        ...
        self.funcNameLister.visit(tree)
        self.funcNames = self.funcNameLister.funcNames
        self.imports = self.funcNameLister.imports
        self.importsFrom = self.funcNameLister.importsFrom
        self.importStars = self.funcNameLister.importStars

        self.ifCount = self.funcNameLister.ifCount
        self.ternIfCount = self.funcNameLister.ternIfCount

    def getNumOfLines(self):
        if (len(self.analyzer.numOfLines) > 0):
            return max(self.analyzer.numOfLines)
        else: return 0

    def getNumOfFuncs(self):
       return self.funcNameLister.getNumOfFuncs()

    def getDensity(self):
        return self.analyzer.density()

    def getNumFromImports(self):
        return self.funcNameLister.numFromImports()

    #after each iteration, the parser needs to reset its states
    def resetStates(self):
        self.analyzer.resetState()
        self.funcNameLister.resetState()
...
class Analyzer(ast.NodeVisitor):
    ...
    #visit() gets called on all nodes
    def visit(self, node):
        self.nodeCount += 1
        try:
...
class FuncLister(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        # TODO make funcNames into array, every funcName has an index
    ...
    def visit_ImportFrom(self, node):
        ...
        if node.module is None and node.level: #check if relative and how many dots
            ...
            if moduleName not in self.importsFrom:
                # print(node.module, "'.' level: ", node.level)
    ...
    def getNumOfFuncs(self):
        return len(self.funcNames.split()) #empty arg is split on space
